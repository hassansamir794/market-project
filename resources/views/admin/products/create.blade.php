            ...
            <form method="POST" action="{{ route('admin.products.store') }}" enctype="multipart/form-data" class="space-y-5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input class="input-clean" name="stock" type="number" step="1" min="0" value="{{ old('stock', 0) }}" required>
                    </div>

                    <div>
                        <label class="field-label">Price ({{ $currencySymbol }})</label>
                        <input class="input-clean" name="price" type="number" step="{{ $priceStep }}" min="0" value="{{ old('price') }}" required>
                    </div>
                </div>

                <div class="flex items-center gap-3">
                    <input id="is_available" name="is_available" type="checkbox" value="1" class="h-4 w-4" @checked(old('is_available', true))>
                    <label for="is_available" class="font-semibold">Available for sale</label>
                </div>

                <div>
                    <label class="field-label">Description</label>
                    <textarea class="textarea-clean" name="description" rows="4">{{ old('description') }}</textarea>
                </div>
                    ...
                    <div class="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-2">
                        @foreach($categories as $cat)
                            <label class="flex items-center gap-3 rounded-xl border border-stone-200/80 bg-white/70 px-3 py-2">
                                <input type="checkbox" name="category_ids[]" value="{{ $cat->id }}" class="h-4 w-4" @checked(in_array($cat->id, old('category_ids', [])))>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <p class="mt-2 text-sm text-gray-600">Used as the main image on product cards.</p>
                    </div>
                    ...
                    <div>
                        ...
                        <p class="mt-2 text-sm text-gray-600">Add extra images for the product detail page.</p>
                    </div>
                </div>
